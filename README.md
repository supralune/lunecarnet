...
- Markdown notes with published/updated metadata, H1–H3 table of contents, tags, and adjacent navigation
- Obsidian-style inline/display LaTeX and icon callouts, including `[!note]+` and `[!warning]-` folding
- Paginated home and category detail pages, plus yearly archive detail pages
...
2. `src/config/blog.ts` contains blog copy and home/category pagination sizes.
...
H1, H2, and H3 headings are included in the article table of contents.
```
...
> [!tip]+ Expanded and foldable
> Add `+` to start open, or `-` to start closed.
```
...
- The search page embeds a small static index and searches titles, metadata, and article bodies entirely in the browser.
- Canonical and Open Graph metadata are generated by the shared layout; article pages expose publication and optional modification dates.
