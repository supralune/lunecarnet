...
模板不包含参考项目中的个人信息或品牌内容。学术部分采用学科中立的通用占位信息，可直接作为 GitHub Template Repository 使用。
...
1. 使用此模板创建仓库并克隆到本地。
2. 在 `src/config/shared.ts` 修改身份信息。
...
6. 选择对应的 `dev:*` 和 `build:*` 命令；部署时配置 `SITE_MODE`。
...
未填写 URL 的论文链接不会渲染，也不会生成 `href="#"` 空链接。删除全部示例文章后，首页、归档和分类页会显示有意义的空状态，不会留下空白区域。
